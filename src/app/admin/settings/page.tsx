...
export default function AdminSettings() {
  ...
  const [features, setFeatures] = useState({ flashSales: true, reviews: true, wishlist: true, compareProducts: true, sellerChat: true, selfRegistration: true });
    ...
    <div>
      <div className={styles.pageHeader}>
        <div>
          ...
          <p className={styles.pageSub}>Global platform configuration and controls</p>
        </div>
      </div>
      ...
      <div className={styles.settingSection}>
        ...
        <div className={styles.settingSectionBody}>
          ...
          <button className="btn btn-primary btn-sm" onClick={savePlatform}>Save Changes</button>
        </div>
      </div>
        ...
        <div className={styles.settingSectionBody}>
          <div className={styles.settingRow}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexShrink: 0 }}>
              <input type="range" min="1" max="30" step="0.5" value={commission} onChange={e => setCommission(Number(e.target.value))}
      ...
      <div className={styles.settingSection}>
        ...
        <div className={styles.settingSectionBody}>
          <div className={styles.formGrid}>
            {Object.entries(shipping).map(([province, fee]) => (
              <div key={province} className={styles.formGroup}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <input
                    ...
                    type="number"
                    value={fee}
                    onChange={e => setShipping(prev => ({ ...prev, [province]: Number(e.target.value) }))}
                  />
                </div>
              </div>
            ))}
          ...
          {([
            ...
            <div key={key} className={styles.settingRow}>
              ...
              <label className={styles.toggleSwitch}>
                <input type="checkbox" checked={payments[key]} onChange={e => setPayments(p => ({ ...p, [key]: e.target.checked }))} />
                <span className={styles.toggleSlider} />
              </label>
            </div>
          ))}
          <button className="btn btn-primary btn-sm" onClick={() => showToast('Payment methods saved')}>Save</button>
        </div>
      </div>
      ...
      <div className={styles.settingSection}>
        ...
        <div className={styles.settingSectionBody}>
          ...
          <button className="btn btn-primary btn-sm" onClick={() => showToast('Feature settings saved')}>Save Features</button>
        </div>
      </div>
      ...
      <div className={styles.settingSection} style={{ borderColor: maintenance ? 'rgba(239,68,68,0.5)' : undefined }}>
        ...
        <div className={styles.settingSectionBody}>
          <div className={styles.settingRow}>
            ...
            <label className={styles.toggleSwitch}>
              <input type="checkbox" checked={maintenance} onChange={e => { setMaintenance(e.target.checked); showToast(e.target.checked ? 'Maintenance mode ENABLED — site is now offline for users' : 'Maintenance mode disabled — site is live'); }} />
